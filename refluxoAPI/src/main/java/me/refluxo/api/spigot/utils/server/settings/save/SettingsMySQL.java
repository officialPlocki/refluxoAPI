...
public class SettingsMySQL {
    ...
    public static void setSettingToValue(APIPlayer apiPlayer, Settings setting, Values value) {
        ...
        try {
            PreparedStatement ps = con.prepareStatement("UPDATE settings SET "+setting.name()+" = '"+value.name()+"' WHERE uuid = '"+p.getUniqueId().toString()+"'");
    ...
    public static Values getSettingValue(APIPlayer apiPlayer, Settings setting) {
        Player p = apiPlayer.getPlayer();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT "+setting.name()+" FROM settings WHERE uuid = '"+p.getUniqueId().toString()+"'");
    ...
    public static void check(APIPlayer apiPlayer) {
        Player p = apiPlayer.getPlayer();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM settings WHERE uuid = '"+p.getUniqueId().toString()+"'");
            ResultSet rs = service.getResult(ps);
            if(!rs.next()) {
